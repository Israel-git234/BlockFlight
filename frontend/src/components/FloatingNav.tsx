...
const navItems = [
  ...
  { id: 'blockflight', icon: Plane, label: 'Flight Mode', color: 'cyan' },
  { id: 'market', icon: Users, label: 'Predictions', color: 'purple' },
...
const getButtonClass = (color: string, isActive: boolean) => {
  ...
  switch (color) {
    case 'blue':
      return 'bg-gradient-to-br from-blue-500 to-blue-600 border-blue-400 shadow-blue-500/25'
    case 'cyan':
      return 'bg-gradient-to-br from-cyan-500 to-cyan-600 border-cyan-400 shadow-cyan-500/25'
    case 'purple':
      return 'bg-gradient-to-br from-purple-500 to-purple-600 border-purple-400 shadow-purple-500/25'
    case 'green':
      return 'bg-gradient-to-br from-green-500 to-green-600 border-green-400 shadow-green-500/25'
    case 'yellow':
      return 'bg-gradient-to-br from-yellow-500 to-yellow-600 border-yellow-400 shadow-yellow-500/25'
    default:
      return 'bg-gradient-to-br from-gray-500 to-gray-600 border-gray-400 shadow-gray-500/25'
  }
}
...
export function FloatingNav({ activeFeature, setActiveFeature }: FloatingNavProps) {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.8, delay: 1.2, type: "spring", stiffness: 300, damping: 20 }}
    >
                ...
                return (
                  ...
                  >
                    ...
                    <Button
                      ...
                      className={`w-14 h-14 rounded-2xl shadow-xl backdrop-blur-sm transition-all duration-300 relative overflow-hidden group border-2 ${getButtonClass(item.color, isActive)}`}
                    >
                      {/* Background Glow */}
                      <motion.div
                        className={`absolute inset-0 ${getGlowClass(item.color)} opacity-0 group-hover:opacity-20 transition-opacity duration-300`}
                      />
                      {/* Icon */}
        ...
        >
          ...
          {[...Array(3)].map((_, i) => (
            <motion.div
              ...
              animate={{ rotate: [0, 360], scale: [0, 1, 0] }}
